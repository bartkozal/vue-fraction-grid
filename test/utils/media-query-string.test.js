...
test('return proper media query string on provided config', () => {
  ...
  mqs = mediaQueryString({ approach: 'mobile', query: '320px 414px' })
  expect(mqs).toEqual("(min-width: 320px) and (max-width: 414px)")

  mqs = mediaQueryString({ approach: 'mobile', query: '415px' })
  expect(mqs).toEqual("(min-width: 415px)")

  mqs = mediaQueryString({ approach: 'desktop', query: '414px' })
  expect(mqs).toEqual("(max-width: 414px)")

  mqs = mediaQueryString({ approach: 'desktop', query: '415px 1400px' })
